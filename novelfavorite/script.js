...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(`https://lestialv.ddns.net:3001/api/FavoritesNovel?userId=${userId}`)
    .then(res => res.json())
    .then(novels => {
      ...
      novels.forEach(novel => {
        ...
        card.innerHTML = `
          ...
          <p class="novel-author">${truncateText(novel.description, 100)}</p>
          <div class="novel-divider"></div>
          <p class="novel-stat">รวมตอน : ${novel.Count ?? 0}</p>
          <p class="novel-stat">สถานะ : ${novel.status}</p>
          <span class="wishlist-icon" style="cursor:pointer; font-size: 24px; color: red;">♥</span>
        `;
        ...
        // คลิก wishlist-icon เพื่อลบออกจากรายการโปรด
        card.querySelector(".wishlist-icon").addEventListener("click", function (e) {
          ...
          .then(res => res.json())
          .then(() => {
            card.remove(); // ลบ card ออกจาก DOM
          });
        });
